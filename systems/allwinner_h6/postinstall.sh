...
# for some strange reason mesa is broken on ubuntu - both the shipped 22.0 as well
...
# update: as panfrost is disabled by default via blacklist in modprobe.d now this
#         is no longer needed
#if [ "${3}" = "focal" ] || [ "${3}" = "jammy" ]; then
