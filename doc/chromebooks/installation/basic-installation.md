...
```
_Note: If deploying for the 3.10 kernel for the early Chromebook Snow releases, you must add `-O ^extref,^skinny-metadata,^no-holes,^zoned -R ^free-space-tree` to disable features not supported by Linux 3.10.
